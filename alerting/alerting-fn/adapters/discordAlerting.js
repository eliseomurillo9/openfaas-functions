async function discordalerting(warning) {
    ...
    const alertsText = warning.alerts
        .map((alert, index) => {
            return [
                `**Alert ${index + 1}**`,
                `Level: ${alert.level}`,
    ...
    const message = {
        ...
        embeds: [
            {
                title: `Sensor ${warning.id}`,
                color: 15158332,
                fields: [
                    {
                        name: "Time",
                        value: warning.time ?? new Date().toISOString(),
                        inline: false,
                    },
                    {
                        name: "Alerts",
...
module.exports = { discordalerting };
